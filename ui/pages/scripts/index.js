/* ───────────────────────── Bridge ───────────────────────── */
  ...
  async function openPluginMarketplace() {
    let plugins = [];
  ...
  async function openInstalledPlugin(pluginId) {
    ...
    if (!canOpen) return;
    let loaded;
    try { loaded = await bridge({ action:'dgt_load_plugin_ui', pluginId }); }
    catch (e) { return toast('err', e.message); }
  ...
  window.__dgtPluginBridge = async function(pluginId, payload) {
    return await bridge({ action:'dgt_plugin_call', pluginId, payload });
  };
